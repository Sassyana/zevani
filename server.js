...
async function sendApp(req,res){
  try{
    ...
    let html;
    if(rebuild){
      // The rebuild site's actual home page is the root index.html.
      // Keep the companion library at /rebuild/ and the chat at /meet.html.
...
async function sendMeet(req,res){try{const html=await fs.readFile(path.join(__dirname,"meet","index.html"),"utf8");res.type("html").send(html)}catch(e){console.error(e);res.status(500).send("ZEVANI could not load the companion chat.")}}
app.get("/meet",sendMeet);app.get("/meet/",sendMeet);app.get("/meet.html",sendMeet);
