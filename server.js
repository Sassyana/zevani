...
async function sendApp(req,res){
  try{
    ...
    const file=rebuild?"index.html":"index-current.html";
    let html=await fs.readFile(path.join(__dirname,file),"utf8");
    if(!rebuild) html=html.replace("</head>",'<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script><link rel="stylesheet" href="/companion-library.css"><script src="/auth-voice-bridge.js"></script><script defer src="/companion-library.js"></script><script defer src="/companion-gender-pronouns.js"></script><script defer src="/companion-avatar-sync.js"></script><script defer src="/companion-save-profile.js"></script></head>');
    res.type("html").send(html)
  }catch(e){console.error(e);res.status(500).send("ZEVANI could not load the frontend.")}
}
app.get("/",sendApp);app.get("/index-current.html",sendApp);app.get("/home.html",sendApp);

async function sendMeet(req,res){try{const file=req.path.startsWith("/rebuild/")?"rebuild/meet.html":"meet/index.html";const html=await fs.readFile(path.join(__dirname,file),"utf8");res.type("html").send(html)}catch(e){console.error(e);res.status(500).send("ZEVANI could not load the companion chat.")}}
app.get("/meet",sendMeet);app.get("/meet/",sendMeet);app.get("/meet.html",sendMeet);app.get("/rebuild/meet.html",sendMeet);

async function sendLibrary(req,res){try{const file=req.path==="/rebuild/library.html"?"rebuild/library.html":"rebuild/index.html";const html=await fs.readFile(path.join(__dirname,file),"utf8");res.type("html").send(html)}catch(e){console.error(e);res.status(500).send("ZEVANI could not load the companion library.")}}
app.get("/rebuild",sendLibrary);app.get("/rebuild/",sendLibrary);app.get("/rebuild/library.html",sendLibrary);
