...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!key) {
      return res.status(503).json({ error: "OPENAI_API_KEY is not configured on this deployment." });
    }
    ...
    return res.status(200).json({
      ok: true,
      reply: response.output_text || "I'm here with you. Tell me more."
    });
  } catch (error) {
    console.error("ZEVANI OpenAI error:", error);
    return res.status(500).json({
      error: error?.message || "The AI could not respond right now. Please try again."
    });
  }
}
